...
employee = { "Gopika" : "gopika", "Sandra" : "sandra", "Jerin" : "jerin", "Ann" : "ann", "Manoj" : "manoj"}


#connection to database
conn = pyodbc.connect('Driver={SQL Server Native Client 11.0};'
                      'Server=LAPTOP-L5C9AERB\MY_INSTANCE1;'
...
@cross_origin()
def Students():
    data2 = pd.read_sql("SELECT * FROM student", conn)
...
@cross_origin()
def Branches():
    data3 = pd.read_sql("SELECT * FROM subject", conn)
...
@cross_origin()
def Details():
    data4 = pd.read_sql("SELECT * FROM Teacher", conn)
